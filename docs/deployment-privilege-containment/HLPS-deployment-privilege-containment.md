...
| U-10 | **Does anything consume the deployment credential config keys?** SD-3a is a denylist with no off switch; if a script re-uses a credential to reach a downstream system, SD-3a breaks it on the first deployment after release. | User | **RESOLVED for non-production; open for production.** Consumption exists, so SD-3a's password half is a migration rather than a drop-in. Non-prod migration is two scripts. Production is unscanned. | **DB slice RESOLVED, and non-empty.** `FOIT_CondaProxyIdentity` interpolates `$DORC_NonProdDeployUsername$` across three property values. **No row in either table references either password.** This splits SD-3a — see the revised direction: passwords are denylisted unconditionally (nothing consumes them), usernames remain script-visible (something does, and a username is an identity rather than a credential). **Share grep COMPLETE for the non-production script folder — 11 matching lines across 10 files, all within `\\Itss.global\prod\GLO\APP\DORC\Scripts\Scripts.ST\`** (the `prod` element is DFS namespace; this is the non-prod DOrc instance's `ScriptRoot`). **The production instance's script folder has not been scanned**, so the production migration size is still unknown — see the residual below. The distribution vindicates the username/password split:

**Username consumers (unaffected by SD-3a as designed).** Seven hits sit at line 6 in `Test-RequiredP…` calls inside "Setup logon as a service" basebuild scripts (`137 AMP`, `146 TREX`, `147 Power Desk`, `151 FOM`, `152 FOP`), plus `RecycleEnv.ps1` and two parameter declarations in `DeploySSRSPowerBIReports.ps1`. Granting a logon right needs the account *name*, not its password. Because SD-3a keeps usernames visible, **none of these require migration** — which is the whole return on making that split rather than denylisting all keys uniformly.

**Probable password consumers — two files, and these gate the password denylist:** `139 UploadersDownloaders\ControllApp…:11` (`$password = C…`) and `51 TFS\TFS Refresh\020 - Hosts File.ps1:83` (`$password …`). Both read as an assignment from an injected credential variable, most likely wrapped in `ConvertTo-SecureString`. The captured lines are truncated, so which key each matched is **inferred, not confirmed** — see the residual note below the register.
