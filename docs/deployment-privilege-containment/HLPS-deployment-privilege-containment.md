...
**Confirmed against a live estate — the worst case is the actual case.** The inventory returned the following. **Provenance: these results are from the System Test DOrc instance (`DeploymentOrchestrator_ST` on `GMDV-DORSQS01\DOR_DV2016`), not the production instance.** The production instance's `ConfigValue` table is unscanned, so the figures below describe ST's copy of the configuration. Whether ST's `DORC_ProdDeployPassword` row holds a real production credential or a stand-in is itself unknown (**U-13**).
...
This instance holds **1,286 environments — 1,084 non-production and 202 production**. Within it, the row named `DORC_ProdDeployPassword` is delivered in cleartext, as an ordinary PowerShell variable, to the deployment scripts of all 1,084 non-production environments: the lower-trust population, where the bar to landing a component is lowest.

**This is not a latent risk in the instance measured — it is an active, maximal disclosure there.** Whether the production instance is configured the same way is the single most valuable outstanding question in this document (**U-13**), because the mechanism is identical and only the data differs. Rotation scope is addressed in SD-3; an interim containment that needs no code is proposed as SD-0.
...
3. **U-13** - query production's `ConfigValue` table. Read-only, and decides whether W-4 is a live production incident.
