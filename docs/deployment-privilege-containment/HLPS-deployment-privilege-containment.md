...
**The inventory converted W-4 from a design weakness into a live production incident.** `DORC_ProdDeployPassword` carries `IsForProd = NULL`, so the production deployment password is currently delivered in cleartext to the deployment scripts of all 1,083 non-production environments. Rotation of the deployment passwords is no longer only a sequencing precondition of this plan — it is an outstanding remediation in its own right, and the plan's value is partly that it stops the rotated credential from being re-disclosed immediately.
