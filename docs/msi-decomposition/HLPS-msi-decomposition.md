...
- **C-10 — Installs serialise per server, and order matters.** `_MSIExecute` is a machine-wide mutex: the four packages cannot install concurrently on one server. Independent does not mean simultaneous. Order is also load-bearing, and now fixed by two separate dependencies: Monitor configuration embeds `DEPLOYAPI.ENDPOINT` and the Monitor calls the API at runtime, so Monitors must not precede the API; and per C-16 the UI depends on certificates the API owns. The resulting order is **API → UI → Monitors → CLIs**, with the API's sidecar carrying the legacy `ProductNames` so the monolith is removed first.
...
- **C-16 — The UI depends on the API, by decision.** Per U-12 the API package owns both certificates and the UI references them. This buys a four-package split instead of five, at the cost of coupling: **"UI installed, API not" is no longer a valid state**, uninstalling the API removes the certificate backing the UI's HTTPS binding, and the API must be installed before the UI (C-10). G-2 should be read accordingly — the UI and API are independently *deployable* in the forward direction only. If that coupling later proves unacceptable, the fifth prerequisites package is the escape hatch, and it also resolves C-7 and the `RegistryEntries` half of C-8.
...
| U-12 | Who owns the two certificates, given they cannot be refcounted (C-8)? | User | — | **RESOLVED** (2026-08-03). **The API package owns both** `DorcNonProdSSLCert.pfx` and `deploymentportal.pfx`; the UI references them. Keeps the split to four packages. Consequence recorded as C-16: the UI now depends on the API, so "UI installed, API not" ceases to be a valid state and uninstalling the API breaks the UI's HTTPS binding. |
| U-13 | Fixed or auto (`*`) `ProductCode`s, and is `MajorUpgrade` introduced? | User | — | **RESOLVED** (2026-08-03). **Fixed `ProductCode` and `UpgradeCode` per package; no `MajorUpgrade`.** Upgrade continues to mean uninstall-then-install by product name through `DeployMSI`, matching the existing mechanism and the maintenance window confirmed in U-3. This also avoids the hazard flagged in round 1: `MajorUpgrade` scheduling interacts directly with the destructive `RemoveFolders` component (C-7), and not introducing it removes that interaction entirely. Legacy removal is handled by naming `Deployment Orchestrator` and `DevOps Orchestrator` in the `ProductNames` of the package sequenced first (C-10). |
...
| U-15 | How does a run express *which* installers it covers? | Agent | **Proposed** — confirm before SD-6 is specified | **PROPOSED.** Reuse the existing `Exclude_MSI` deny-list (`RunDeployment.ps1:8,123`) and derive the covered set as its complement over `MsiFileNames`. It is already plumbed through the pipeline as a variable, needs no new configuration surface, and keeps "deploy everything" as the default. An include-list would be more explicit but adds a second selector for the same job. |
| U-16 | How are services associated with installers? `DeploymentServices` is one flat string covering both monitors. | Agent | **Proposed** — confirm before SD-6 is specified | **PROPOSED.** Move the association into each `.msi.json` sidecar, which is already the home for per-installer metadata (`ServerType`, `ProductNames`, `Parameters`). Add a `Services` array: the Monitors sidecar lists both action services, the others list none. The quiesce step then stops only the services belonging to installers in the covered set, and the global `DeploymentServices` string is retired. |
...
**No blocking unknowns remain.** U-9 and U-11 closed by reading the deploy module; U-4 and U-7 from `Product.wxs`; U-1, U-2, U-3, U-12 and U-13 by user decision. U-15 and U-16 carry proposed resolutions that should be confirmed before SD-6 is specified, but neither now halts the IS. Per `CLAUDE.md` the IS may proceed.
