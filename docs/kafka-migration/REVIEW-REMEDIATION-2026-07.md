...
| # | Finding | Fix |
|---|---------|-----|
| 1 | Prod and NonProd Monitor services install with the same lock consumer group (`dorc.monitor.locks.nonprod`); the runbook referenced a `Kafka:Locks:ConsumerGroupId` MSI parameter that did not exist. Lock partitions split across tiers → deployments to environments hashing to the other tier's partitions stall forever. | `KAFKA.LOCKS.CONSUMERGROUPID.PROD` / `.NONPROD` MSI parameters with tier-distinct defaults, wired through the split-package `Install.Orchestrator.bat`, `Setup.Dorc.Monitors.msi.json`, `DeploySettings.template.json`, and both Monitor `ActionService.wxs` files. Runbook updated to describe the real mechanism. |
