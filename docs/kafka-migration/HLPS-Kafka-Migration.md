...
| **C-8** | **Poison-message handling: per-source DLQ Kafka topic (amended 2026-04-30).** **Original wording (superseded):** DB error-log table — premised on "the target Kafka deployment does not support DLQ features." **Re-evaluation per PR #611 K-2** showed nothing prevents producing to an additional Kafka topic; the per-source DLQ pattern is now the substrate. **Current contract:** on a message-processing failure (deserialization error, business-logic exception, or repeated failure), DOrc MUST: (a) produce an Avro `KafkaErrorEnvelope` to the DLQ topic configured for the source topic, capturing at minimum — source topic, partition, offset, consumer group, message key, raw payload (truncated if oversize, with a flag), exception type/message/stack, timestamps; (b) commit the offset; (c) continue processing the stream. Consumer groups must never be stalled by a single failing message. **If the DLQ produce fails OR no DLQ is configured for the source topic** (e.g. by deliberate scope cut — currently only `RequestsNew` has a route), the consumer MUST fall back to structured-log capture of the same fields and still commit the offset. **Super-degraded:** if the structured-log itself throws, swallow and commit. **Retention:** Kafka topic retention (`retention.ms`, default 30 d on `Kafka:ErrorLog:RetentionMs`) replaces the SQL purge-job from the original wording. **Replay:** standard Kafka consumer tooling against the DLQ topic. See `SPEC-S-004 §0` for the substrate-change details and `SPEC-S-006 R-8` / `SPEC-S-007 R-3` for the consumer-side three-tier model. |
