...
| Field | Value |
|---|---|
| Status | **APPROVED** — by the owner, 2026-08-02, after three panel rounds (triages §12–§14). U-16 resolved in the same decision. Governs `IS-responsive-record-view.md`. |
| Date | 2026-08-02 |
| Owner | Ben Hegarty |
...
| **U-16** | Runtime a11y assertion mechanism: none exists (`eslint-plugin-lit-a11y` is static; `grid-menu-a11y` declined to add a harness) and §4 forbids new test frameworks. | User | ~~Blocks §6 step 6~~ | **RESOLVED 2026-08-02 by the user: axe-core approved** — as an assertion library inside the existing Vitest/Playwright browser harness, not a new test framework, so §4's constraint is honoured. SC5's automated half runs on axe assertions; the manual screen-reader spot-check remains. SC8 note: axe-core is a dev dependency added under this explicit approval. |
...
| — | 2026-08-02 | **APPROVED** | Owner | **User approval granted.** U-16 resolved: axe-core approved as an assertion library in the existing browser harness. Next artifact: `IS-responsive-record-view.md`. |
