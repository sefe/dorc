...
|-----|-------------|-------|----------|--------|
| U-1 | What is the intended permission model for Terraform plan view/confirm/decline (A-1)? Assumed: same as request-lifecycle confirm — user must have modify rights on the deployment's environment. Confirm this mirrors `CanModifyEnvironment`, and whether confirm requires a higher privilege (e.g. environment owner) than view. | User | **Blocking A-1** | Open. Proposed default: view = read access to the env; confirm/decline = `CanModifyEnvironment`. |
| U-2 | Is the `fn:` C# expression evaluator (B-2) a used, supported feature? If yes, what is the legitimate use case and who is authorised to author such expressions? | User | **Blocking B-2** | Open. Default if unused: remove entirely. If used: sandbox behind an explicit config flag with a restricted `ScriptOptions` (no imports, no host object). |
