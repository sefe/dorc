...
| U-2 | Is the `fn:` C# expression evaluator (B-2) a used, supported feature? | User | **Blocking B-2** | **RESOLVED (2026-07-05).** It is an **actively-used, required** feature of DOrc properties. It must **not** be removed. S-002 becomes a hardening step: constrain the evaluator's capabilities (restricted `ScriptOptions` — no arbitrary imports, no host object, no file/process/network access), and where feasible restrict *who* can author `fn:` values via existing property permissions. A residual-risk note is required since sandboxing `CSharpScript` is imperfect. See S-002 for the security sub-questions this raises (Q-2a..Q-2c). |
