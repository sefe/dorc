...
> **Implementation note (2026-07-06) — Direction A, delivered.** U-2 resolved: `fn:` is used only for simple string/maths ops, so `CSharpScript` is now gated by a fail-closed Roslyn AST allow-list (`SafeExpressionValidator`) before execution: only literals, arithmetic/comparison/logical operators, the ternary, and allow-listed instance members on values or any static member of `Math`/`Convert`. 5s eval timeout; blocking `.Result` replaced.
>
> **Round-2 frontier panel (Opus 4.8 + Fable 5) found a CRITICAL bypass in the first cut and it is now fixed:** `"".GetType()` — a Unicode-escaped `GetType` whose raw `Identifier.Text` ≠ `"GetType"` slipped the *deny*-list but the compiler still bound the real `GetType()`, opening full reflection (empirically wrote a file to disk). Fixes: (1) compare `Identifier.ValueText` (compiler-bound name), not `.Text`; (2) invert to an **allow-list** of member names so reflection entry points (`GetType`, `Assembly`, `GetMethod`, `MakeGenericType`, `DynamicInvoke`, …) are absent by construction; (3) reject generic member names. The panel also flagged a DoS (`"x".PadRight(2000000000)` → ~4GB alloc the cooperative timeout can't interrupt) — `PadLeft`/`PadRight` are excluded from the instance allow-list. Post-fix, both reviewers' full attack sets (incl. the weaponized reflection chains) are rejected; delivered with `SafeExpressionValidatorTests` (regression rows for the escaped-`GetType` chains and the DoS). **A round-3 re-attack (Fable) confirmed no HIGH/CRITICAL bypass survives** and surfaced that `Math`/`Convert` expressions actually throw `CS0103` at runtime (the scripting host is called with no imports) — verified empirically. So the only expressions that ever executed are string methods on literals and arithmetic operators. The validator was therefore **tightened to reject all bare-identifier receivers** (removing the `Math`/`Convert` blanket branch): this matches runtime reality, removes the reviewer's "safety rests on missing imports" concern, and loses no working behaviour. **Known compat limitation (documented):** predefined-type statics (`int.Parse`, `string.Format`), string interpolation, and indexers are rejected fail-safe (visible error, not silent); extend the allow-list — with `.WithImports` if statics are wanted — if the property audit shows they're used. Authorship permission-gating remains a defence-in-depth follow-up.
...
> **Round-2 review fixes (2026-07-05).** (1) `GetLog` originally guarded only on `GetRequestForUser` non-null — the Opus reviewer correctly noted that method performs no permission filtering (it only computes a `UserEditable` flag), so it was an existence check, not authorization. `GetLog` now requires `CanModifyEnvironment` on the owning environment (the full log can echo script output/paths). (2) `IsValidUncPath` hardened per both reviewers: rejects `..` traversal, `:` (drive-qualified paths and NTFS alternate data streams), and drive-letter administrative shares (`\\server\C$\…`), while still allowing named hidden shares. Test rows added for each.
...
> **Implementation note (2026-07-05).** E-1 (the disclosure) is fixed at the single choke point: `ExceptionJsonConverter` no longer serializes the stack trace, the inner-exception chain, the fully-qualified type, **or the raw exception message** — it emits only `{ Type=Name, Message="An unexpected error occurred" }`. **Round-2 frontier review** (Opus 4.8 + Fable 5) flagged that the top-level `ExceptionMessage` is itself the common leak vector (e.g. `SqlException` "Login failed for user 'X'… database 'Y'") and that the first test only checked the *inner* message; the message is now dropped from both the converter **and** `DefaultExceptionHandler` (still logged server-side), and a top-level-message test was added. Also fixed the one site that bypassed the converter via `exception.ToString()` (`RequestStatusesController.Get`). **Remaining (E-5, quality):** convert validation-failure 500s to 400, add targeted NRE guards, and sweep the pervasive `return …(ex.Message)` string returns that bypass the converter (F-2 from round-2) — tracked as follow-up.
...
> **Implementation note (2026-07-05).** D-3 done: both PowerShell runners (`Dorc.PowerShell` and `Dorc.NetFramework.PowerShell`) no longer `JsonConvert.SerializeObject(property.Value)` on a `SetVariable` failure — they log only the variable name and its type (the value may be a decrypted secret and the log flows to file + OpenSearch). The Terraform-output masking half of D-2 is handled with the Terraform work in **S-010** (it needs the secret-value set to mask against), and the on-disk tfvars half is also S-010.
...
> **Implementation note (2026-07-06) — partial (C-3 done).** The exit-code interpretation is now command-aware via `TerraformProcessor.IsTerraformCommandSuccessful(exitCode, arguments)`: a `plan -detailed-exitcode` treats **0 and 2** as success (2 = "succeeded, there are changes" — the normal case) and 1/other as failure; every other command fails on any non-zero code. This fixes C-3 (a crashed/OOM `apply` with exit ≠ 1 was previously reported success) **without** the regression both round-1 reviewers flagged (a blanket "non-zero = fail" would mark every changed plan Failed). Delivered with `TerraformExitCodeTests` (10 cases). **Still deferred (Windows/integration, paired with S-012):** real `CancellationToken` threading + process-tree kill (C-4), stdout drain before read (C-11), and `finally`-based tfvars cleanup (D-2 on-disk) — these need Windows process control and an integration harness this sandbox lacks.
