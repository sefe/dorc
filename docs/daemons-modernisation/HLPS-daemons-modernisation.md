...
- **C-02** No breaking change to consumers of daemon data. `DaemonApiModel.ServiceType`, `VariableValueDaemons.ServiceType`, and the TypeScript-generated bindings for these models keep their current field names and types. The C# property `Daemon.ServiceType` is also preserved — only the underlying column is renamed. `[StringLength(50)]` → `[StringLength(250)]` is a model-validation loosening (previously-rejected payloads are now accepted); this is additive / back-compatible per C-02's intent.
  - **Amendment 2026-04-24 (mid-execution, user-approved)**: an additional wire-level rename was approved during S-005 execution — `ServiceStatusApiModel` class renamed to `DaemonStatusApiModel`, with JSON field renames `ServiceName` → `DaemonName` and `ServiceStatus` → `Status`. This is technically a breaking wire change and **not** purely additive, but is permitted here because U-2 and U-5 confirmed zero out-of-repo consumers of the daemon-status endpoints (scanned both `dorc2` and `dorc-cli`). `ErrorMessage` is new/additive alongside the rename. If an external consumer is later discovered, the fix is to add `[JsonPropertyName("ServiceName")]` / `[JsonPropertyName("ServiceStatus")]` / `[JsonPropertyName("ServiceStatusApiModel")]` compatibility aliases; the C# refactor itself stays.
