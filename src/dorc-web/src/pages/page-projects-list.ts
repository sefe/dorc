...
export class PageProjectsList extends ResponsiveMixin(PageElement) {
        ...
        : html`
            ...
            >
              <vaadin-grid-column
                ...
                flex-grow="0"
                header=""
                ${columnBodyRenderer(this._sourceControlTypeRenderer, [])}
              ></vaadin-grid-column>
              <vaadin-grid-sort-column
                path="ProjectName"
                header="Name"
                resizable
                ${columnBodyRenderer(this._projectNameRenderer, [])}
              ></vaadin-grid-sort-column>
              <vaadin-grid-sort-column
  ...
  _projectEnvsButtonsRenderer(
    item: ProjectApiModel
  ) {
    const project = item as ProjectApiModel;
    return html` <project-controls
        .project="${project}"
        .deleteHidden="${!this.isAdmin}"
      ></project-controls>`;
  }
  ...
  _sourceControlTypeRenderer(
    ...
    if (isGitHub) {
      ...
      return html`<svg title="GitHub" width="${size}" height="${size}" viewBox="0 0 98 96" xmlns="http://www.w3.org/2000/svg" style="vertical-align: middle;">
      ...
      return html`<vaadin-icon icon="vaadin:folder-open" title="File Share" style="color: ${color}; width: ${size}px; height: ${size}px;"></vaadin-icon>`;
    } else {
      // Azure DevOps logo (use flat color to avoid SVG id conflicts across rows)
      return html`<svg title="Azure DevOps" width="${size}" height="${size}" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg" style="vertical-align: middle;">
  ...
  _projectNameRenderer(
    ...
    const project = item as ProjectApiModel;
    return html`<span title="${project.ProjectDescription || ''}">${project.ProjectName}</span>`;
  }
