...
import { ServerAuditApi, PagedDataSorting } from '../apis/dorc-api';
import { PagedDataFilter } from '../apis/dorc-api/models';
import { ServerAuditApiModel } from '../apis/dorc-api/models/ServerAuditApiModel';
...
export class PageServersAudit extends PageElement {
  ...
  render() {
    return html`
      ...
      <vaadin-grid
        ...
        .dataProvider="${this.getServerAudit}"
      ...
      >
        <vaadin-grid-column
          path="ServerName"
          header="Server"
          ${columnBodyRenderer(this.serverNameRenderer, [])}
          resizable
          auto-width
          flex-grow="0"
        ></vaadin-grid-column>
  ...
  private valueRenderer(fieldName: 'FromValue' | 'ToValue') {
    return (item: ServerAuditApiModel) => {
  ...
  private refreshGrid() {
    this.dispatchEvent(
      new CustomEvent('searching-server-audit-started', {
        detail: {},
        bubbles: true,
        composed: true
      })
  ...
  getServerAudit = (
    params: GridDataProviderParams<ServerAuditApiModel>,
    callback: GridDataProviderCallback<ServerAuditApiModel>
  ) => {
    ...
    api
      ...
      .subscribe({
        next: (data: GetServerAuditListResponseDto) => {
          data.Items?.forEach(
            item => (item.Username = getShortLogonName(item.Username))
          );
          this.dispatchEvent(
            new CustomEvent('searching-server-audit-finished', {
              detail: {},
              bubbles: true,
              composed: true
            })
        ...
        complete: () => {
          this.dispatchEvent(
            new CustomEvent('server-audit-loaded', {
              detail: {},
              bubbles: true,
              composed: true
            })
