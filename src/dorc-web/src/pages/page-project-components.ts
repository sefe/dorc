...
export class PageProjectComponents extends ResponsiveMixin(PageElement) {
  ...
  @property({ type: Array })
  filteredComponents: ComponentDeploymentInfo[] = [];
  ...
  @property({ type: Array })
  environments: EnvironmentApiModel[] = [];

  @property({ type: Map })
  envBuildsMap: Map<string, EnvironmentContentBuildsApiModel[]> = new Map();
  ...
  render() {
    return html`
            ...
            : !this.refDataLoading && !this.environmentsLoading
              ? html`
                  <div class="no-selection">
                    Select a component from the dropdown to view deployment
                    information
                  </div>
                `
              : ''
      ...
      <div
        class="loader"
        ?hidden="${!this.refDataLoading && !this.environmentsLoading}"
      ></div>
    `;
  }
  ...
  private loadBuildsForEnvironment(envName: string): Promise<void> {
    return new Promise(resolve => {
      ...
      api.refDataProjectBuildsGet({ id: envName }).subscribe({
        ...
        error: (err: any) => {
          console.error(
            `Error loading builds for environment ${envName}:`,
            err
          );
  ...
  private handleComponentSelected(
    e: ComboBoxSelectedItemChangedEvent<ComponentDeploymentInfo>
  ) {
  ...
  private updateDeploymentRows() {
    ...
    this.deploymentRows = this.environments.map(env => {
      const build = this.selectedComponent!.environmentBuilds?.get(
        env.EnvironmentName!
      );
  ...
  private applyFilter() {
    ...
    this.filteredComponents = this.flattenComponents(
      this.components,
      [],
      new Set<string>(),
      childIds
  ...
  private flattenComponents(
    ...
    sortedComponents.forEach(component => {
      ...
      const hasScript = Boolean(
        component.ScriptPath && component.ScriptPath.trim() !== ''
      );
