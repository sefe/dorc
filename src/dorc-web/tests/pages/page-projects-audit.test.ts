...
const { mockRefDataProjectAuditPut } = vi.hoisted(() => {
  ...
  const mockRefDataProjectAuditPut = vi.fn(() => ({
...
import { PageProjectsAudit } from '../../src/pages/page-projects-audit';

type OverviewHost = {
  updateOverviewViewport: (pane: HTMLElement, viewport: HTMLElement) => void;
  onOverviewClick: (e: MouseEvent) => void;
  onOverviewKeydown: (e: KeyboardEvent) => void;
};
...
function makePane(scrollHeight: number, clientHeight: number, scrollTop = 0) {
  ...
  // Real browsers clamp scrollTop to 0 on an empty element. Override the
  // property so tests can supply a virtual scroll position.
  let top = scrollTop;
  ...
  pane.scrollTo = vi.fn((opts: ScrollToOptions) => {
    top = opts.top ?? 0;
...
describe('PageProjectsAudit overview ruler', () => {
  ...
  let overview: OverviewHost;

  beforeEach(async () => {
    el = new PageProjectsAudit();
    document.body.appendChild(el);
    await el.updateComplete;
    overview = el as unknown as OverviewHost;
  });
  ...
  describe('updateOverviewViewport', () => {
    it('hides the viewport indicator when content fits without scrolling', () => {
      ...
      overview.updateOverviewViewport(pane, viewport);
  ...
  describe('onOverviewClick', () => {
    ...
    it('scrolls to (ratio * scrollHeight) - clientHeight/2 on click', () => {
      ...
      const ruler = mountOverview(pane);
      const ev = { currentTarget: ruler, clientY: 50 } as unknown as MouseEvent;

      overview.onOverviewClick(ev);
    ...
    it('clamps the scroll target to 0 when the click would land above the top', () => {
      ...
      const pane = makePane(1000, 200);
      const ruler = mountOverview(pane);
      const ev = { currentTarget: ruler, clientY: 5 } as unknown as MouseEvent;
    ...
    it('does nothing when no scroll pane sibling is present', () => {
      ...
      expect(() => overview.onOverviewClick(ev)).not.toThrow();
    });
  ...
  describe('onOverviewKeydown', () => {
    ...
    function mountWithMarkers(
      pane: HTMLDivElement,
      markerTopPercents: number[]
    ) {
      const wrapper = document.createElement('div');
      const ruler = document.createElement('div');
      ruler.className = 'diff-overview';
    ...
    it('Enter jumps to the next change marker after the viewport centre', () => {
      ...
      const pane = makePane(1000, 200, 0);
      const ruler = mountWithMarkers(pane, [10, 50, 90]);
      const ev = new KeyboardEvent('keydown', { key: 'Enter' });
      Object.defineProperty(ev, 'currentTarget', { value: ruler });

      overview.onOverviewKeydown(ev);
    ...
    it('does nothing when there are no markers', () => {
      const pane = makePane(1000, 200, 0);
      const ruler = mountWithMarkers(pane, []);
      const ev = new KeyboardEvent('keydown', { key: 'Enter' });
      Object.defineProperty(ev, 'currentTarget', { value: ruler });
