...
namespace Dorc.Core.AzureDevOpsServer
{
    public interface IAzureDevOpsServerWebClient
    {
        ...
        Task<List<Build>> GetBuildsFromDefinitionsAsync(string collection, List<BuildDefinitionReference> buildDefinitions, int requestSize = 10, CancellationToken cancellationToken = default);
        ...
        Task<List<Build>> GetBuildsFromBuildNumberAsync(string collection, string buildNumber, string projectName,
            int requestSize = 10, CancellationToken cancellationToken = default);
    }
}