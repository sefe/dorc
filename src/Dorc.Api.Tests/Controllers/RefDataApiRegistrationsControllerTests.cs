...
namespace Dorc.Api.Tests.Controllers
{
    ...
    /// </summary>
    [TestClass]
    public class RefDataApiRegistrationsControllerTests
    {
        private IApiRegistrationsPersistentSource _source = null!;
        private IApiRegistrationAuditPersistentSource _audit = null!;
        ...
        private RefDataApiRegistrationsController _controller = null!;

        [TestInitialize]
        public void Setup()
        {
            _source = Substitute.For<IApiRegistrationsPersistentSource>();
            _audit = Substitute.For<IApiRegistrationAuditPersistentSource>();
            ...
            _controller = new RefDataApiRegistrationsController(
        ...
        {
            var result = _controller.Post(new ApiRegistrationApiModel { Name = "web" });

            Assert.AreEqual(403, StatusOf(result));
            _source.DidNotReceive().Add(Arg.Any<ApiRegistrationApiModel>());
        ...
        {
            MakePowerUser();
            _source.GetByName("web").Returns(new ApiRegistrationApiModel { Id = 1, Name = "web" });
        ...
        {
            MakePowerUser();
            _source.GetByName("web").Returns((ApiRegistrationApiModel?)null);
            _source.Add(Arg.Any<ApiRegistrationApiModel>()).Returns(new ApiRegistrationApiModel { Id = 9, Name = "web" });

            var result = _controller.Post(new ApiRegistrationApiModel { Name = "web" });

            Assert.AreEqual(200, StatusOf(result));
            _audit.Received(1).InsertApiRegistrationAudit(@"DOM\alice", ActionType.Create, 9,
        ...
        public void Put_UnattachedItem_WithoutRole_Returns403()
        {
            ...
            var result = _controller.Put(9, new ApiRegistrationApiModel { Name = "web" });

            Assert.AreEqual(403, StatusOf(result));
            _source.DidNotReceive().Update(Arg.Any<int>(), Arg.Any<ApiRegistrationApiModel>());
        }
        ...
        public void Put_WithEnvWrite_UpdatesAndAuditsBeforeAfter()
        {
            _source.GetEnvironmentNamesForId(9).Returns(new List<string> { "DV 01" });
            GrantEnvWrite("DV 01");
            _source.GetById(9).Returns(new ApiRegistrationApiModel { Id = 9, Name = "old" });
            _source.GetByName("new").Returns((ApiRegistrationApiModel?)null);
            _source.Update(9, Arg.Any<ApiRegistrationApiModel>())
                .Returns(new ApiRegistrationApiModel { Id = 9, Name = "new" });

            var result = _controller.Put(9, new ApiRegistrationApiModel { Name = "new" });

            Assert.AreEqual(200, StatusOf(result));
            _audit.Received(1).InsertApiRegistrationAudit(@"DOM\alice", ActionType.Update, 9,
        ...
        public void Put_UnknownId_Returns404()
        {
            MakePowerUser();
            _source.GetEnvironmentNamesForId(9).Returns(new List<string>());
            _source.GetById(9).Returns((ApiRegistrationApiModel?)null);

            Assert.AreEqual(404, StatusOf(_controller.Put(9, new ApiRegistrationApiModel { Name = "x" })));
        }
        ...
        public void Put_NameConflictWithOtherItem_Returns409()
        {
            ...
            _source.GetEnvironmentNamesForId(9).Returns(new List<string>());
            _source.GetById(9).Returns(new ApiRegistrationApiModel { Id = 9, Name = "old" });
            _source.GetByName("taken").Returns(new ApiRegistrationApiModel { Id = 1, Name = "taken" });

            Assert.AreEqual(409, StatusOf(_controller.Put(9, new ApiRegistrationApiModel { Name = "taken" })));
        }
        ...
        public void Delete_WithEnvWrite_DeletesAndAudits()
        {
            ...
            Assert.AreEqual(200, StatusOf(result));
            _audit.Received(1).InsertApiRegistrationAudit(@"DOM\alice", ActionType.Delete, 9,
        ...
        public void Detach_HappyPath_DetachesAndAudits()
        {
            ...
            Assert.AreEqual(200, StatusOf(result));
            _audit.Received(1).InsertApiRegistrationAudit(@"DOM\alice", ActionType.Detach, 9,
