...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="wix:File[@Source='$(var.DorcMonitorDir)\appsettings.json']/@Id">
		<xsl:attribute name="{name()}">
			<xsl:value-of select="'NonProdDeployMonitorExeConfig'" />
		</xsl:attribute>
	</xsl:template>

	<xsl:template match="wix:File[@Source='$(var.DorcMonitorDir)\log4net.config']/@Id">
		<xsl:attribute name="{name()}">
			<xsl:value-of select="'NonProdLog4NetConfig'" />
		</xsl:attribute>
	</xsl:template>
