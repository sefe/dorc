...
/// <summary>
/// Scripted <see cref="IAdminClient"/> for the provisioner's error-policy
/// tests. Only CreateTopicsAsync and GetMetadata are scriptable — the only
/// members the provisioner touches; everything else throws.
/// </summary>
internal sealed class ScriptedAdminClient : IAdminClient
{
    /// <summary>Invoked for CreateTopicsAsync; throw or return a (possibly hung) task.</summary>
    public Func<IEnumerable<TopicSpecification>, Task>? OnCreateTopics { get; set; }

    /// <summary>Invoked for GetMetadata(topic, timeout).</summary>
    ...
    public Task CreateTopicsAsync(IEnumerable<TopicSpecification> topics, CreateTopicsOptions? options = null)
        => OnCreateTopics?.Invoke(topics) ?? Task.CompletedTask;
