...
{
    // S-008 runtime: resolves a stock-template reference (catalog name +
    ...
    public class CatalogReferenceCodeSourceProvider : ITerraformCodeSourceProvider
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            // The sub-path within the repository is the module dir at repo root.
            // Convention: stock-modules/<name>/. The catalog manifest could
            // override this in future via a parameter, but today we keep it
            // simple and convention-driven.
            if (string.IsNullOrEmpty(scriptGroup.TerraformSubPath))
            {
                scriptGroup.TerraformSubPath = $"stock-modules/{manifest.Name}";
            }
